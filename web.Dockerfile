...
LABEL \
    ...
    io.nanobox.logs.2.prefix="apache" \
    io.nanobox.logs.2.path="/var/log/web/out.log"
...
COPY .docker/before_live_web /opt/nanobox/hooks/before_live

COPY .docker/before_live_all /opt/nanobox/hooks/before_live_all

COPY .docker/after_live_web /opt/nanobox/hooks/after_live
